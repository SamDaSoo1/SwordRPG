...
namespace WindowsInput
{
    public class WinInput
    {
        ...
        private static bool[] m_PrevKeyState = new bool[(int)KeyCode.Joystick8Button19];
        private static bool[] button_up = new bool[(int)KeyCode.Joystick1Button19];
        ...
        public static bool GetKeyDown(KeyCode key)
        {
            ...
            {
                m_PrevKeyState[(int)key] = false;
                return false;
            }
            else if (m_PrevKeyState[(int)key] == false)
            {
                m_PrevKeyState[(int)key] = true;
                button_up[(int)key] = true;
                return true;
            }
            return false;
#else
...
#if (UNITY_STANDALONE_WIN && !UNITY_EDITOR_OSX)
        int keyState = GetAsyncKeyState(KeyCodeToVkeyFullSet(key));
            if (keyState == 0 && button_up[(int)key] == true)
            {
                button_up[(int)key] = false;
                return true;
            }
            else return false;
        }
